...
In your setup, you should replace `OUT_DIR` with your desired build configuration of either `Debug`, `Testing`, and `Release`, and `out_dir.gn` correspondingly with `debug.gn`, `testing.gn` or `release.gn`.
...
$ gn gen out/OUT_DIR --args="import(\"//electron/build/args/out_dir.gn\") $GN_EXTRA_ARGS"
```
